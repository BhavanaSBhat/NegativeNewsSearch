...
def fetch_news_newscatcher_api(query_string : str, from_date : str, to_date : str, language : str, no_of_articles:int):
    articles_page_size = 10 # page size defaulted to 10
    ...
    while len(total_fetached_articles) < 2:
        ...
        total_fetached_articles.extend([{"full_text" : article['summary']} for article in fetched_articles])
        page_no = page_no + 1
        time.sleep(1)
    return total_fetached_articles

def fetch_news_twitter_api(query_string:str, language:str, from_date:str, no_of_articles:int, tweet_mode='extended'):
    ...
    list_tweets = tweepy.Cursor(tweety_api.search, q=query_string, lang=language, since=from_date, tweet_mode=tweet_mode).items(no_of_articles)
    fields_to_fetch = ['full_text', 'entities']
    total_tweets = []
    for tweet in list_tweets:
        sub_tweet_dict = {}
        for field_name in fields_to_fetch:
            sub_tweet_dict[field_name] = sub_tweet_dict[tweet._json[field_name]]
        total_tweets.append()
    return total_tweets

#testing parameters
# query_string = '(Apple AND company) OR "Apple Inc"'fetch_news_newscatcher_api('apple', '22-10-2022', '15-11-2022', 'en', 10)
